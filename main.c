 ...
 */
void input_password_hidden(char *password) {
    int i = 0;
    char ch;
    while ((ch = _getch()) != '\r') { // Читаем до нажатия Enter (\r)
        if (ch == '\b') { // Обработка Backspace
            if (i > 0) { 
                i--; 
                printf("\b \b"); // Стираем звездочку из консоли
            }
        } else {
            password[i++] = ch; 
            printf("*"); // Выводим маску
        }
    }
...
void ShowGraphWindow(Point* pts, int count, int degree) {
    ...
    char title[200];
    sprintf(title, "График интерполяции (Порядок: %d) | Идентификатор процесса: %lu | Идентификатор потока: %lu",
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(in_log, login) == 0) {
        printf("Пароль: ");
        input_password_hidden(in_pwd); // Ввод со звездочками
        if (double_hash(in_pwd) != saved_hash) {
            printf("Ошибка. Неверный пароль.\n"); return 1;
        }
    ...
    double x_tmp, y_tmp;
    // ИСПРАВЛЕНО: Считываем в x_tmp и y_tmp
    while (fscanf(f, "%lf %lf", &x_tmp, &y_tmp) == 2 && count < MAX_POINTS) {
        if (x_tmp >= -1000.0 && x_tmp <= 1000.0 && y_tmp >= -1000.0 && y_tmp <= 1000.0) {
            pts[count].x = x_tmp;
            pts[count].y = y_tmp;
